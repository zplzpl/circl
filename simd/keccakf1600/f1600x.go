...
// Keccak, SHA3 and SHAKE. Running two, four, or eight permutations in parallel is
...
// The recommendation is to beforehand verify IsEnabledX4(), IsEnabledX2(),
// and IsEnabledX8() to determine if the current system supports the SIMD implementation.
package keccakf1600
...
type StateX4 struct {
	...
	// If true, permute will use 12-round keccak instead of the usual 24.
	turbo bool
}
...
type StateX8 struct {
	// Go guarantees a to be aligned on 8 bytes, whereas we need it to be
	// aligned on 32 bytes for better performance. Thus we leave some headroom
	// to be able to move the start of the state.

	// 8 x 25 uint64s for the interleaved states and three uint64s headroom
	// to fix alignment.
	a [203]uint64
...
// IsEnabledX8 returns true if the architecture supports an eight-way SIMD
// implementation provided in this package.
func IsEnabledX8() bool { return cpu.X86.HasAVX512F }
...
// on {a[0], a[8], a[16], ...}, the second on {a[1], a[9], a[17]}, etc.
// If turbo is true, applies 12-round variant instead of the usual 24.
...
// Permute performs the eight parallel Keccak-f[1600]s interleaved on the slice
// returned from Initialize().
func (s *StateX8) Permute() {
	if IsEnabledX8() {
		permuteSIMDx8(s.a[s.offset:], s.turbo)
	} else {
		permuteScalarX8(s.a[s.offset:], s.turbo) // A slower generic implementation.
	}
}
...
func permuteScalarX8(a []uint64, turbo bool) {
	var buf [25]uint64
	for i := 0; i < 8; i++ {
		for j := 0; j < 25; j++ {
			buf[j] = a[8*j+i]
